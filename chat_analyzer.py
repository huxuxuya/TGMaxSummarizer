...
class ChatAnalyzer:
    ...
    def _create_reflection_prompt(self, summary: str, messages: List[Dict], chat_context: Dict) -> str:
        ...
        prompt = f"""РОЛЬ: Эксперт по анализу текста, НЕ переписываешь текст, а только анализируешь его.

ЗАДАЧА: Проанализируй данную суммаризацию и дай конкретные советы по улучшению.

ВАЖНО: НЕ переписывай суммаризацию! Только анализируй и давай советы.

КРИТЕРИИ ОЦЕНКИ:
- Полнота: все ли важное включено?
- Четкость: понятны ли действия?
- Структурированность: логично ли организовано?
- Краткость: нет ли лишней информации?
- Точность: не добавлена ли несуществующая информация?
...
ДАЙ АНАЛИЗ (НЕ ПЕРЕПИСЫВАЙ ТЕКСТ):
**Полнота**: что упущено?
**Четкость**: что неясно?
**Точность**: есть ли несуществующая информация?
**Улучшения**: что можно исправить?

ПРИМЕРЫ ПРАВИЛЬНОГО ОТВЕТА:
**Полнота**: Не указано точное время мероприятия
**Четкость**: Неясно, где именно проходит собрание
**Точность**: Все указанные детали соответствуют исходным сообщениям
**Улучшения**: Добавить адрес и контактный телефон

ПРИМЕРЫ НЕПРАВИЛЬНОГО ОТВЕТА:
❌ Переписывание всей суммаризации
❌ Создание новой структуры текста
❌ Добавление новой информации

ПРАВИЛА: Максимум 2-3 предложения по каждому пункту. Только конкретные предложения.
...
- Используй обычные переносы строк вместо <br>"""
    ...
    def _create_improvement_prompt(self, original_summary: str, reflection: str, messages: List[Dict], chat_context: Dict) -> str:
        ...
        prompt = f"""Улучши суммаризацию по предложениям:
...
- Разделы с предложениями по улучшению удалить
- Только улучшенная версия текста"""
